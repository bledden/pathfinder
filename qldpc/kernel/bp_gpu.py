"""Device-agnostic torch edge-list GPU BP (the stage-(a) GPU baseline).

A torch normalized-min-sum belief-propagation decoder on the SAME CSR/CSC edge
layout ``bb_layout.tanner_adjacency`` produces and ``bp_baseline.BpBaseline``
consumes -- but **batched over shots** and runnable on CPU now / CUDA later with
only ``device=``. It reproduces ``BpBaseline`` BIT-IDENTICALLY for one iteration
(same four-phase normalized-min-sum math, same two-smallest-magnitude exclude-self
min, same sign-product-of-others, same coset sign). This is the honest GPU
baseline and the latency reference the later Triton circulant-unroll kernel
(T7b-b) must beat.
...
init mu = prior LLR. Config matches ``BpBaseline``: ``ms_scaling_factor=0.625``,
``max_iter=30``.
...
class BpGpu:
    ...
    def from_dem(cls, dem, max_iter=30, ms_scaling_factor=0.625,
                 dtype=torch.float64):
        """Build from a shared stim DEM via canon_dem.extract (matched protocol).

        Stores ``.dem`` and the observable map ``Lo`` so ``decode_batch`` returns
        predicted observables, mirroring BpBaseline / the zoo adapter surface.
        """
    ...
    def decode_batch(self, dets, device="cpu"):
        ...
        Same interface as BpBaseline.decode_batch / the zoo adapters: each shot's
    ...
    # ------------------------------------------------------------------ #
    # Latency benchmark (RUN ON THE POD GPU later; CPU path works now)    #
    # ------------------------------------------------------------------ #
    @classmethod
    def bench_latency(cls, dem, shots, device="cuda", n_warmup=10, n_iter=100,
        ...
        locally; this is the pod GPU reference.
        """
        import time
