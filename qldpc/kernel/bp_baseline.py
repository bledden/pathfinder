"""Portable normalized-min-sum BP baseline (the stage-(a) reference).
...
``bb_layout.tanner_adjacency`` produces and that the later Triton
"circulant-unroll" kernel (T7b) will consume. The per-iteration update is written
as the four GPU-portable phases the kernel will fuse:
...
the shape a GPU kernel needs, so this is the reference the kernel matches
bit-identically for one iteration.
...
Schedule: parallel (flooding). Config matches the zoo BP adapter:
...
class BpBaseline:
    ...
    Args:
        ...
        ms_scaling_factor: normalized-min-sum scale (0.625 matches the zoo BP).
    ...
    def from_dem(cls, dem, max_iter=30, ms_scaling_factor=0.625,
        ...
        predicted observables, mirroring the zoo adapter surface.
        """
    ...
    def decode_batch(self, dets):
        ...
        Same interface as the zoo adapters: each shot's error estimate e_hat maps
