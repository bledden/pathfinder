...
memory**, run as a **relay ensemble of legs**. It is the strongest *practical*
decoder in our zoo (smallest gap-to-MLE) and the only one NVIDIA CUDA-Q QEC 0.6
ships as a production GPU decoder -- but CUDA-only. This is an OPEN, PORTABLE
Triton implementation (one MI300X/ROCm port away) that is LER-matching the
``relay_bp`` Rust oracle.

It is built ON TOP of ``bp_triton.py``'s fused min-sum check/bit-update kernels:
...
LER-IDENTITY (the bar) is reported by ``tests/kernel/test_relay_triton.py`` and
``bench_relay_triton.py``: the gamma-draw RNG differs from the Rust oracle's
internal RNG and fp differs, so per-shot agreement is reported alongside the
logical-error COUNT match.
"""
import numpy as np
