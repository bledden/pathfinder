"""Triton min-sum BP kernel (the stage-(b) GPU kernel target).

A Triton normalized-min-sum belief-propagation decoder that reproduces
``bp_baseline.BpBaseline`` / ``bp_gpu.BpGpu`` for one flooding iteration and is
benchmarked against the torch edge-list baseline (``BpGpu``) on the H200.

DESIGN -- "circulant / bounded-degree unroll", shots-major.
=========================================================
The original spike framing ("circulant-unroll over compile-time (a,b) shifts,
weight-6 checks / weight-3 bits") describes the *abstract* BB code ``HZ``. The
*actual* decoding target is the circuit-level **DEM** Tanner graph (from
``canon_dem.extract``), which is irregular: for the canonical
``build_memory(BBCode(), 6, 0.003, 'Z', 'si1000')`` DEM the check degrees are
13 or 20 and bit degrees are 2 or 3 (n_checks=252, n_bits=1584, nnz=4536). It is
NOT block-circulant in a way a fixed (a,b)-shift unroll exploits.

So this kernel generalizes the circulant-unroll idea to the real DEM: instead of
compile-time circulant shifts we precompute, per check (resp. per bit), the set
of its incident **edge indices padded to the max degree** (MAXDEG_C / MAXDEG_B)
with a sentinel. The kernel then has a *compile-time-bounded, regular* neighbor
loop per row -- the algebraic analogue of unrolling a fixed number of directions
-- with mask-on-load for the padding. This keeps the per-row access
...
``run_iterations_batch``, ``decode_batch``, ``bench_latency``) so the existing
...
from .bb_layout import tanner_adjacency

try:
    import triton
    import triton.language as tl
    _HAVE_TRITON = True
except Exception:  # pragma: no cover - local Mac has no triton
    triton = None
    tl = None
    _HAVE_TRITON = False
...
# ======================================================================= #
# Triton kernels (compiled only on the pod; never touched locally).        #
# ======================================================================= #
if _HAVE_TRITON:
