"""Statistics / verdict layer over the matched-grid manifests.

Turns matched cross-decoder results into defensible verdicts per the
pre-registration (``qldpc/zoo/prereg.json``):
  ...
  * ``gap_to_mle_bootstrap`` (R2, BINDING) -- the LOCKED gap-to-MLE statistic =
    ...
    on the SAME sampled shots (prereg ``gap_statistic = per_shot_ratio_bootstrap``;
    ``aggregate_ratio_single_wilson = false``). Resamples shot INDICES with
  ...
  * ``multiplicity_grid`` -- Holm-Bonferroni (primary, FWER) + BH-FDR (secondary)
    ...
    (prereg ``report_full_grid_including_losses = true``).
...
except ImportError:  # loaded flat (qldpc/ on path)
...
def gap_to_mle_bootstrap(decoder_fail_mask, anchor_fail_mask, n_boot=10000, seed=0):
    ...
    This is the locked gap-to-MLE statistic (prereg ``per_shot_ratio_bootstrap``):
...
def multiplicity_grid(comparisons):
    ...
    Returns:
        ...
        losses; prereg ``report_full_grid_including_losses = true``) where each
        comparison gains:
