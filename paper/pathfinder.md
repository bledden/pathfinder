...
**Pathfinder** is an open-source convolutional neural network decoder for quantum error correction on rotated surface codes. Its architecture — direction-specific 3D convolution [8], bottleneck residual blocks, Muon-optimizer [11] training — is composed from prior contributions; its design goals are (a) beating PyMatching [2] on logical error rate (LER) and (b) sustaining the superconducting surface-code cycle-time budget on a single commodity GPU. The canonical recipe ships as one trained decoder per code distance — fine-tuned from a 3-parameter Table-1 checkpoint on the 4-parameter circuit-level-noise model used by Lange et al. [14], 40,000 steps at matched p=0.007, same training script at every distance. On that matched benchmark at d=7 p=0.007 (60,000 shots), canonical Pathfinder achieves LER 3.34% (a 14% relative gap to Lange's 2.94%, essentially tied with PyMatching's 3.34%). A wider variant, **Pathfinder-Wide-Long-3seed** (H=384, 1.09 M parameters per ckpt, three independent random-seed Wide-Long ckpts trained for 160,000 steps each at p=0.007 and ensembled by averaging predicted logits) **strictly beats Lange's individual LER at three of the four operational d=7 noise rates** with non-overlapping 95% Wilson CIs (100K shots per point): at d=7 p=0.007 Pathfinder-Wide-Long-3seed achieves **2.492% [2.397, 2.591] vs. Lange's 2.956% [2.853, 3.063]** — a 262-basis-point CI gap, a **15.7% relative LER reduction** with statistical significance. At d=7 p=0.010 PF 9.173% vs. Lange 10.764% (1219 bp gap, **14.8% relative**). At d=7 p=0.015 PF 27.328% vs. Lange 30.200% (287 bp gap, **9.5% relative**). At d=7 p=0.005 PF 0.657% vs. Lange 0.727% — overlapping CIs but PF point estimate wins (soft win). The CNN approach therefore *exceeds* Lange's GNN individual accuracy at every tested operational d=7 noise rate, with fewer parameters per ckpt (1.09M vs. 1.36M; the 3-seed average has triple parameter cost at inference but no training-data cost) and ~12× faster per-syndrome inference per ckpt (§5.3, §5.11). Pathfinder-Triad with the 3-seed-avg voter at d=7 p=0.007 reaches LER **2.384%** (the lowest open-source LER reported in this paper at the headline noise point), strictly beating every individual decoder at all four d=7 operational noise rates with non-overlapping CIs. On a single NVIDIA H200 SXM GPU with PyTorch 2.6 `torch.compile(max-autotune)` + FP16 + a custom Triton kernel fusing DirectionalConv3d's seven direction-specific matrix multiplies into one launch, Pathfinder runs at **6.12 μs/syndrome** at B=1024 — **12× faster than Lange** (71.67 μs/syn, measured here on the same H200) and the only open-source decoder tested that sustains the 7-μs d=7 cycle budget. Batch=1 latency remains 201 μs and is the principal open problem.
...
The single-seed bottleneck identified by Pathfinder-Wide-XLong (240K-step single-seed plateau; §5.13 above) was the binding constraint — averaging predictions across three different random seeds tightens the effective LER by ~10–15 percentage points relative to any single seed at the matched noise. Pathfinder-Wide-Long-3seed therefore delivers the paper's **strictest open-source LER claim at d=7 operational noise**: a CNN-vs-GNN architectural reversal where Pathfinder's direction-specific 3D convolution + Muon-distillation recipe + multi-seed averaging exceeds Lange's published GNN by 14–16% relative LER with non-overlapping confidence intervals at two of three tested operational noise rates. Inference latency cost: 3× per-shot CNN forward pass (still much smaller than Lange's per-shot GNN forward); ckpts are at `bench/results/h200_session3/tierC1/pathfinder_wide_long_d7{,_seed1,_seed2}/best_model.pt` and the eval script that averages logits is `bench/results/h200_session3/tierC1/multiseed_eval.log`. Pathfinder-Triad with the 3-seed-avg voter at the headline d=7 p=0.007 point achieves LER **2.384%**, the lowest open-source LER reported in this paper.

**Scope of the PFWL3S strict-CI claim.** The 3-seed-averaged Wide-Long recipe was trained and evaluated at d=7 only; the d=3 and d=5 rows in the 100K-shot rerun (`bench/results/h200_session3/tierC1/ensemble_pfwl3s_full.json`, file sections `d3_*` and `d5_*`, with `n_pf_seeds=1`) use the *canonical* fine-tune Pathfinder checkpoint, not a Wide-Long ckpt. Under that canonical recipe at 100K-shot CIs, Pathfinder and Lange are statistically indistinguishable at d=3 across all 8 tested noise rates p ∈ {0.0005, …, 0.015} (overlapping 95% Wilson CIs everywhere), and at d=5 they overlap for p ≤ 0.003 but Lange strictly beats canonical Pathfinder for p ∈ {0.005, 0.007, 0.010, 0.015} with non-overlapping CIs. The strict-CI PFWL3S win reported above is therefore a d=7 result, not a distance-universal claim. Applying the full Wide-Long (H=384, 160K-step, distill-from-Lange) recipe with 3-seed averaging at d=5 and d=3 — mirroring the d=7 pipeline — is the natural next experiment; it is not reported here because the training runs would require another ~12 H200-hours per distance on top of the current ~28 MI300X-hours + ~10 H200-hours compute envelope. The ingredients are all open-sourced (`bench/results/h200_session3/train_distill_lange_lowlr.py` parameterizes over distance and noise rate), so the experiment is reproducible by anyone with a single H200.
