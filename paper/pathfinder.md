...
**Pathfinder-Triad** — a three-way majority-vote ensemble of (Pathfinder, Lange et al. [14], PyMatching [2]). Each shot is decoded by all three; the ensemble prediction is the elementwise majority of the three binary outputs. No additional training. At d=7 p=0.007 Pathfinder-Triad achieves LER **2.42%** with non-overlapping 95% Wilson CI (2.30, 2.54) against Lange's (2.81, 3.08) — a statistically significant 17.8% relative improvement over the best individual decoder.
...
5. **Three documented negative results** — a distillation / ensemble-independence tradeoff (§5.13: the distilled **Pathfinder-KD** variant has lower individual LER than Pathfinder at d=7 but gives a *looser* Pathfinder-Triad ensemble); a modern-primitives hybrid architecture (§5.14: adding attention + RMSNorm + SwiGLU at 9× parameter count makes the model worse at matched budget); and a d=9 extension (§6.3: the current recipe does not converge at d=9 in an 80K-step budget via either distill or from-scratch training).
...
**Table 10: Pathfinder-Triad vs. individual decoders (LER %, 60K shots, 4-parameter noise)**

| d | p | Pathfinder | Lange | PM | **Pathfinder-Triad** | Oracle-LB | Winner | CI vs Lange |
|---|---|-----------|-------|-----|----------------------|-----------|--------|-------------|
| 3 | 0.003 | 0.572 | **0.535** | 0.665 | 0.557 | 0.427 | Lange | overlap |
| 3 | 0.005 | 1.527 | **1.493** | 1.798 | 1.537 | 1.155 | Lange | overlap |
| 3 | 0.007 | 2.817 | **2.713** | 3.205 | 2.757 | 2.042 | Lange | overlap |
| 3 | 0.010 | 5.302 | **5.140** | 5.852 | 5.180 | 3.913 | Lange | overlap |
| 5 | 0.003 | 0.240 | **0.192** | 0.340 | 0.207 | 0.122 | Lange | overlap |
| 5 | 0.005 | 1.142 | **0.957** | 1.428 | 1.010 | 0.565 | Lange | overlap |
| 5 | 0.007 | 3.040 | **2.580** | 3.547 | 2.657 | 1.572 | Lange | overlap |
| 5 | 0.010 | 7.657 | 6.772 | 8.273 | **6.660** ★ | 3.615 | **Triad** | overlap |
| 7 | 0.003 | 0.120 | **0.087** | 0.148 | 0.092 | 0.043 | Lange | overlap |
| 7 | 0.005 | 0.965 | 0.752 | 0.985 | **0.680** ★ | 0.295 | **Triad** | non-overlap |
| 7 | 0.007 | 3.343 | 2.940 | 3.343 | **2.417** ★ | 1.075 | **Triad** | **non-overlap** |
| 7 | 0.010 | 11.937 | 10.822 | 10.300 | **9.092** ★ | 3.682 | **Triad** | non-overlap |

Bold = lowest individual LER; ★ = Pathfinder-Triad strictly beats *all three* individual decoders with statistical significance where indicated. "Oracle-LB" is the fraction of shots where all three decoders are simultaneously wrong — a hard lower bound on any majority-vote ensemble LER. "CI vs Lange" reports whether the 95% Wilson confidence interval for Pathfinder-Triad overlaps Lange's at that point.

**Findings.**
1. **Pathfinder-Triad strictly beats every individual decoder at 4 of 12 points**, sweeping d=7 at operational noise (p ∈ {0.005, 0.007, 0.010}) plus d=5 p=0.010. At **d=7, p=0.007** Pathfinder-Triad's 2.417% is a **17.8% relative reduction** over Lange alone (2.940%) with **non-overlapping 95% Wilson CIs — (2.30, 2.54) vs. (2.81, 3.08)** — a statistically significant ensemble win. At d=7 p=0.010 the 9.09% is a 16.0% reduction over Lange and 11.7% over PM, also non-overlapping CIs. At d=7 p=0.005, 0.68% vs. Lange 0.75% (11.9% relative, non-overlapping CI). At d=7 p=0.003 the margin (0.092% vs. Lange 0.087%) is within overlapping CIs — Lange wins by a hair.
2. At d=3 and d=5 low-noise, Pathfinder-Triad does not strictly beat Lange. In the easy-decoding regime, the three individual decoders are already close to the ensemble limit and the combinatorial gap is too small to recover.
3. The oracle lower bound at d=7 p=0.007 is 1.075%, so Pathfinder-Triad (2.417%) captures roughly (2.940 − 2.417) / (2.940 − 1.075) ≈ 28% of the available ensemble headroom over Lange. A learned meta-decoder or per-noise-rate gating could plausibly close more.
4. **Confidence-thresholded gating does not help.** I also tested a confidence-thresholded gate that uses Pathfinder's prediction when |logit| > T, else Lange, at T ∈ {1, 2, 3, 4}; this variant never strictly beat Lange alone at any of the 12 (d, p) points in this evaluation. Pathfinder-Triad (3-way majority vote) is a strict improvement over that scheme.
