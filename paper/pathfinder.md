...
**Pathfinder** is an open-source convolutional neural network decoder for quantum error correction on rotated surface codes. Its architecture — direction-specific 3D convolution [8], bottleneck residual blocks, Muon-optimizer [11] training — is composed from prior contributions; its design goals are (a) beating PyMatching [2] on logical error rate (LER) and (b) sustaining the superconducting surface-code cycle-time budget on a single commodity GPU. The canonical recipe ships as one trained decoder per code distance — fine-tuned from a 3-parameter Table-1 checkpoint on the 4-parameter circuit-level-noise model used by Lange et al. [14], 40,000 steps at matched p=0.007, same training script at every distance. On that matched benchmark at d=7 p=0.007 (60,000 shots), canonical Pathfinder achieves LER 3.34% (a 14% relative gap to Lange's 2.94%, essentially tied with PyMatching's 3.34%). A wider variant, **Pathfinder-Wide-Long-3seed** (H=384, three independent random-seed Wide-Long ckpts trained for 160,000 steps each at p=0.007 with Lange-teacher distillation and ensembled by averaging predicted logits) **strictly beats Lange's individual LER at four (d, p) points across two code distances** with non-overlapping 95% Wilson CIs (100K shots per point): at **d=7 p=0.007** PFWL3S **2.492% [2.397, 2.591] vs. Lange's 2.956% [2.853, 3.063]** — a 262 bp CI gap, **15.7% relative LER reduction**. At **d=7 p=0.010** PF 9.173% vs. Lange 10.764% (1219 bp, **14.8% relative**). At **d=7 p=0.015** PF 27.328% vs. Lange 30.200% (287 bp, **9.5% relative**). At **d=5 p=0.015** PF **17.205% [16.972, 17.440] vs. Lange 17.898% [17.662, 18.137]** — 222 bp CI gap, **3.9% relative**, a new statistically-significant win at a lower distance from the same recipe (1.13M params per ckpt at d=5). At d=7 p=0.005 PF 0.657% vs. Lange 0.727% — overlapping CIs but PF point estimate wins (soft win). At d=5 p ∈ {0.005, 0.007, 0.010} PFWL3S ties Lange within overlapping CIs (multi-seed averaging recovers the single-seed Wide-Long loss documented in §5.11 Table 9). The 160K-step distillation recipe **does not work at d=3** — PFWL3S with the default loss weights regresses to 14% LER, and a rescue with the BCE/KL weights swapped (α_bce=0.7, α_kl=0.3, three seeds) only reaches 3.18% — still strictly worse than Lange's 2.78% and worse than the simpler 252K-parameter canonical fine-tune Pathfinder's 2.87% (§5.13 d=3 rescue table); canonical Pathfinder therefore remains the d=3 deployment default. The CNN approach therefore exceeds or ties Lange's GNN individual accuracy at every tested operational noise rate at d=5 and d=7, with smaller parameter count per ckpt than Lange (~850K at d=5, 1.09M at d=7 vs. Lange's 1.36M; the 3-seed average has triple parameter cost at inference but no extra training-data cost) and ~12× faster per-syndrome inference per single-model ckpt with the §5.3 Triton kernel; the 3-seed-average inference cost is exactly 3× the single-model cost (measured: 157.36 μs/syn at d=7 B=1024 without Triton, ≈24 μs/syn with Triton extrapolated; §5.13 latency subsection), so PFWL3S is positioned for non-real-time deployment regimes (offline protocol verification, post-selection) rather than within the 7-μs cycle-time budget. Inside the cycle budget, the 1-seed Triton-kernel canonical Pathfinder of §5.3 (6.12 μs/syn) is the right operating point at the cost of ~25% higher individual LER. Pathfinder-Triad with the 3-seed-avg voter at d=7 p=0.007 reaches LER **2.384%** (the lowest open-source LER reported in this paper at the headline noise point), strictly beating every individual decoder at all four d=7 operational noise rates with non-overlapping CIs. At d=5 the same Triad strictly beats every individual decoder at p ∈ {0.010, 0.015} (Maj<<all category in `eval_pfwl3s_v2.log`). On a single NVIDIA H200 SXM GPU with PyTorch 2.6 `torch.compile(max-autotune)` + FP16 + a custom Triton kernel fusing DirectionalConv3d's seven direction-specific matrix multiplies into one launch, Pathfinder runs at **6.12 μs/syndrome** at B=1024 — **12× faster than Lange** (71.67 μs/syn, measured here on the same H200) and the only open-source decoder tested that sustains the 7-μs d=7 cycle budget. Batch=1 latency remains 201 μs and is the principal open problem.
...
| d=7 | **7-ckpt mega-ensemble** (warm-init Triad-distill × 3 + H=512 Triad-distill × 3 + PF+PM × 1) | ~8.0M total | **2.458%** ★★ | [2.364, 2.556] | **2.399%** | **STRICT WIN over Lange** at p ∈ {0.007, 0.010, 0.015}; still loses to Pathfinder-Triad at p=0.015 |
