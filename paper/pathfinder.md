...
**Interpretation.** The results are consistent with two compounding effects: (1) Lange's GNN architecture is genuinely stronger at this task than Pathfinder's CNN at matched noise model, as evidenced by its ≈15–30% LER reduction vs. Pathfinder across d=5 and d=7 at p ≥ 0.005; (2) Pathfinder's performance is amplified by the train/test noise mismatch — its LER under the 4-parameter noise model (Table 9) is 2.5–4× higher than under the 3-parameter noise model of Table 1 at d=7, p=0.007 (4.05% vs. 1.04%). How much of the Pathfinder–Lange gap is architectural versus OOD cannot be determined without retraining Pathfinder at 4-parameter noise. A 4-parameter retrain takes ≈5–6 GPU-hours per distance, was deferred from this paper for scheduling reasons, and is the single most important pending experiment.
...
| 5 | 0.003 | 0.253 | 0.192 | 0.340 | 0.213 | 0.120 | Lange |
| 5 | 0.005 | 1.113 | 0.957 | 1.428 | 0.990 | 0.550 | Lange |
| 5 | 0.007 | 2.935 | 2.580 | 3.547 | 2.602 | 1.498 | Lange |
| 5 | 0.010 | 7.728 | 6.772 | 8.273 | **6.668** | 3.602 | **Majority** |
| 7 | 0.003 | 0.103 | 0.087 | 0.148 | **0.083** | 0.038 | **Majority** |
| 7 | 0.005 | 1.082 | 0.752 | 0.985 | **0.677** | 0.293 | **Majority** |
| 7 | 0.007 | 4.010 | 2.940 | 3.343 | **2.563** | 1.057 | **Majority** |
| 7 | 0.010 | 13.575 | 10.822 | 10.300 | **9.362** | 3.535 | **Majority** |

"Oracle-LB" = fraction of shots where all three decoders are simultaneously wrong, a lower bound on any ensemble's LER. "Winner" = strict winner among the four columns (individual < Majority ≤ Majority < individual).

**Findings.**
1. **Majority vote strictly beats every individual decoder at 5 of 12 points**, concentrated at d=7 and at d=5 p=0.010 where the failure modes diverge most. At d=7 p=0.007 the majority vote's 2.56% is a 12.9% relative reduction over Lange alone (2.94%); at d=7 p=0.010 it is a 13.5% relative reduction over Lange (10.82%) and 9.1% over PM (10.30%).
...
3. The oracle lower bound at d=7 p=0.007 is 1.057%, so majority vote (2.563%) captures roughly (2.940 − 2.563) / (2.940 − 1.057) ≈ 20% of the available ensemble headroom over Lange. A learned meta-decoder (or per-noise-rate gating) could plausibly close more.
