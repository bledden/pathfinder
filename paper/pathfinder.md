...
The paper uses two noise models that differ in whether the fourth parameter is set:

- **3-parameter noise (Table 1, §5.1)** sets the first three parameters and leaves `after_reset_flip_probability=0`. This is the noise model the original canonical Pathfinder is trained on.
...
**PFWL3S inference latency at d=7 (measured).** The 3-seed-average inference cost was previously stated as "3× per-shot CNN forward pass". I measured this directly: a single PFWL3S forward pass (H=384, no Triton kernel) at d=7 takes 52.51 μs/syn at B=1024 (3.10 ms total at B=1) on the H200; the 3-seed-average ensemble takes **157.36 μs/syn at B=1024 (3.0× exactly, 9.17 ms at B=1)**. Data: `bench/results/h200_session3/tierC1/pfwl3s_latency.json`; benchmark script: `bench/results/h200_session3/tierC1/bench_pfwl3s_latency.py`. The 3.0× linear scaling is exact across all batch sizes and matches naive expectation (3 forward passes + 1 logit-average). Two implications: (i) without Triton, PFWL3S at d=7 B=1024 is 157 μs/syn — 2.2× slower than Lange's 71.67 μs/syn (§5.11) and well above the 7-μs d=7 cycle budget. (ii) With the §5.3 Triton DirectionalConv3d kernel applied to each of the three forward passes, single-model latency would drop from 52.5 μs to ≈8 μs/syn (extrapolating the 6.5× Triton speedup measured in §5.3 at H=256 and accounting for ~1.5× larger H=384 weights), so 3-seed PFWL3S would be ≈24 μs/syn — still 3× faster than Lange but still 3.5× over the cycle budget. Practical conclusions: the 3-seed-average PFWL3S claims in the abstract refer to a *non-real-time* deployment regime (offline protocol verification, post-selection in repeat-until-success). For real-time d=7 decoding inside the cycle budget, the canonical (1-seed) Triton-kernel Pathfinder configuration of §5.3 (6.12 μs/syn) is the right operating point, accepting the lower individual LER (3.34% canonical vs PFWL3S's 2.49% at d=7 p=0.007) in exchange for cycle-time compliance.
