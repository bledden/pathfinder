...
|---|---:|---:|:---:|:---:|:---:|:---:|
| p=0.007 | **2.533%** | 2.652% | overlap | 701 / 820 | χ²=9.15, **p=0.0025** | **win** (Holm) |
| p=0.010 | **9.140%** | 9.538% | PFWL3S strict | 2653 / 3051 | χ²=27.6, **p=1.5×10⁻⁷** | **win** (Bonf./24) |
...
**The honest reading — and a clean illustration of why the paired test is primary.** Against the toughest control — a full-recipe 3-seed Lange ensemble — PFWL3S wins at **p=0.007 and p=0.010**, but the *two tests disagree about p=0.007*, and the paired test is the correct arbiter. At **p=0.010** both agree: the marginal 95% CIs are non-overlapping *and* McNemar is decisive (p=1.5×10⁻⁷, surviving Bonferroni over all 24 (d, p) comparisons). At **p=0.007** the marginal CIs *overlap* (PFWL3S [2.437, 2.632] vs Lange-FT [2.554, 2.753]) — the marginal test would call it inconclusive — yet the paired McNemar test, which removes the shared-shot variance the marginal test ignores, finds a significant win (p=0.0025, Holm-significant): a concrete case where the correct paired test recovers a real win the weaker marginal test misses. At **p=0.015 both tests agree it is a tie** — marginal CIs overlap and McNemar p=0.092, with PyMatching (27.269%) also matching PFWL3S (27.304%) at that high rate. **The defensible C3 claim is therefore: under the paired McNemar test, PFWL3S significantly beats a full-recipe 3-seed Lange ensemble at p=0.007 and p=0.010 (most robustly at p=0.010), and ties it at p=0.015.** This is the matched-ensemble, paired-test, multiplicity-corrected control; the win is not an ensemble-size or training-budget artifact at the two lower operational rates, and is honestly a tie at the highest. The published-Lange control in the same run reproduces at 3.027% (vs the paper's 2.956%), validating the harness; PFWL3S in this control reads 2.533% (a fresh, independent 100K-shot sample), consistent with the canonical 2.492% headline within shot-noise. Data: `coda_experiments/lange_3seed_eval.json`; eval: `coda_experiments/eval_lange_3seed.py`.
