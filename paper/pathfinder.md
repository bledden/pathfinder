...
| d=3 | Pathfinder-Wide-Long-3seed (H=384, 3-seed avg of 160K-step ckpts) | ~1.71M total (3 × 570K) | **14.01%** ✗ | [13.79, 14.22] | — | **negative result — distillation fails at d=3** (canonical fine-tune is the d=3 default) |
...
**d=3 PFWL3S — does not work; canonical fine-tune is the d=3 default.** Applying the same recipe at d=3 (H=384, 160K steps, distill from Lange at p=0.007) produces a model with **catastrophic individual LER** at every tested noise rate — e.g., 14.01% [13.79, 14.22] at d=3 p=0.007 vs. canonical fine-tune Pathfinder's 2.87% (§5.11 Table 9 d=3 row, 60K shots) and Lange's 2.78%. This is a 5× regression relative to the simpler fine-tune recipe and is consistent with the existing finding (above) that Pathfinder-KD distillation also fails catastrophically at d=3 (13.4% LER at the same point in the 80K-step Pathfinder-KD ablation). The d=3 surface code apparently has too few stabilizers and too shallow an effective receptive field for the H=384 / Lange-teacher distillation regime to converge — the canonical 252K-parameter fine-tune model with BCE loss is the right operating point. **At d=3, canonical Pathfinder remains the recommended decoder**; the §5.11 Table 9 d=3 row (overlapping CIs with Lange across all matched noise rates) stands. PFWL3S is therefore a d=5- and d=7-only construction; d=3 retains canonical fine-tune (§5.11 d=3 OOD-tied-with-Lange numbers). Pathfinder-Triad at d=3 with the canonical fine-tune voter is the correct deployment configuration there.
