...
**Real-hardware validation.** Two real-hardware runs are included: §5.15 evaluates against Google Willow Sycamore d=7 r=13 traces (PyMatching 4.006%; PFWL3S random-prediction baseline due to L=6/9/15 compound-detector input-format mismatch, an *adapter* artifact rather than a noise-OOD finding); §5.15.1 evaluates against **IBM Heron r2 (`ibm_fez`)** under controlled circuit topology with matched rounds, removing both the adapter and rounds-mismatch confounds. With **calibrated multi-noise training** (`train/train_calibrated.py`, 3 seeds × 150K steps, p sampled per-batch from `[0.003, 0.025]`), **PFWL3S ties PyMatching on real IBM Heron r2 at both d=3 r=3 (28.98% vs 28.49%) and d=5 r=5 (47.27% vs 45.68%) with overlapping 95% Wilson CIs** (the d=5 r=5 tie is confirmed at both 10K and 100K shots; §5.15.1) — the first PFWL3S-class neural decoder shown to statistically match PyMatching on real superconducting-chip noise across two code distances. The earlier-draft d=5 r=5 single-p-trained result (47.68%, PM strict-wins 4.4% relative) is documented in Table 13a row 2 as the un-calibrated baseline to show the calibrated-training delta.
...
**Key finding.** Pathfinder + Triton is the only configuration that sustains the d=7 cycle-time budget across all operational noise rates. PyMatching sustains the budget only below p ≈ 0.006–0.007; above that, PM falls progressively behind as noise rises. For deployments where the expected worst-case noise exceeds ~0.006, Pathfinder + Triton is the only decoder in this comparison that is both real-time and accurate. Figure 3 plots the accuracy–latency Pareto at d=7, p=0.007 for every open-source decoder in this paper (Pathfinder variants, Lange, PyMatching) together with the two most-cited closed-source comparators (Gu et al. [8], AlphaQubit [5]) on their reported hardware.
...
![Pathfinder and Lange fail on largely disjoint syndromes (only ~1.8% shot overlap)](../figures/fig05_failure_overlap.png){.fig}

**Figure 5.** *Per-shot decoder agreement at d=7, p=0.007 (60K shots; data from `bench/results/h200_main/phase2/ensemble_results_final.json`).* Venn diagram of the failure sets: Pathfinder is wrong on 1,854 shots, Lange is wrong on 1,764 shots, both are wrong on only 1,083 shots. The two decoders therefore disagree on which shots fail roughly half the time. This near-disjoint failure mode is the structural reason the §5.12 three-way majority vote (Pathfinder-Triad) strictly beats every individual decoder — independent error coverage, not architectural similarity, is what the ensemble exploits.
...
This is reported as a **provisional positive result: a 10K-shot statistical tie with PyMatching at both d=3 r=3 and (after calibrated multi-noise training) d=5 r=5**, with the residual single-p OOD gap and the engineering path that closes it (calibrated-noise training) explicitly documented in Table 13a rows 2–3. It is meaningfully stronger evidence for the §5.11 / §5.13 family than the §5.15 Willow result on its own and supersedes the §5.15 footnote that PFWL3S "fails on real hardware" — that statement was true under input-adapter mismatch and circuit-format mismatch, and is *not* true under matched topology and matched rounds at d=3.
