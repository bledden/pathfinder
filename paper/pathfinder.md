# Pathfinder: A Direction-Aware Neural Decoder that Outperforms the Prior State-of-the-Art Open-Source Neural Decoder on Surface Codes
...
**Real-hardware validation.** Real-hardware validation spans two platforms (Google Willow Sycamore and IBM Heron r2), with a soft-readout follow-up on IBM: §5.15 evaluates against Google Willow Sycamore d=7 r=13 traces (PyMatching 4.006%; PFWL3S random-prediction baseline due to L=6/9/15 compound-detector input-format mismatch, an *adapter* artifact rather than a noise-OOD finding); §5.15.1 evaluates against **IBM Heron r2 (`ibm_fez`)** under controlled circuit topology with matched rounds, removing both the adapter and rounds-mismatch confounds. With **calibrated multi-noise training** (`train/train_calibrated.py`, 3 seeds × 150K steps, p sampled per-batch from `[0.003, 0.025]`), **PFWL3S ties PyMatching on real IBM Heron r2 at d=3 r=3 (28.98% vs 28.49%, overlapping 95% Wilson CIs)** — the first PFWL3S-class neural decoder shown to statistically match PyMatching on real superconducting-chip noise. At d=5 r=5 both decoders sit at the random-guess baseline (47.3% vs 45.7%; observable-flip rate 0.49 — the chip is at/past surface-code threshold there), so that point does not discriminate decoder quality; the calibrated-training delta that brings the single-p CIs to overlap is reported (§5.15.1) but is within the chip's cross-epoch PM drift. The earlier-draft d=5 r=5 single-p-trained result (47.68%, PM strict-wins 4.4% relative) is documented in Table 13a row 2 as the un-calibrated baseline to show the calibrated-training delta. §5.15.2 additionally tests **soft (analog-IQ) readout** — the lever behind AlphaQubit's [5] margin over MWPM — on `ibm_fez` kerneled-IQ data (20K shots/point), and finds it does **not** break the PFWL3S↔PM tie at either distance: d=3 is too clean for soft to matter (1.4% graded detectors; soft ≈ hard within shot-noise) and d=5 is past threshold (all decoders ≈ random), a calibration-robust negative on a clean matching-saturated chip (the AlphaQubit soft-gain regime requires a noisier-readout but still sub-threshold device).

**A note on priority.** Lange et al. [14] (PRR 2025; arXiv:2307.01241) previously released the first open-source neural decoder to outperform PyMatching on rotated surface codes under circuit-level noise. The present work extends Lange's open-source priority with: (a) coverage of operational noise rates p ∈ {0.007, 0.010, 0.015} not in Lange's training distribution; (b) a depth-dependent Muon ablation (§6.2) showing Muon is essential at d≥7; (c) the Triton kernel of System 1 closing the cycle-time gap; (d) PFWL3S and Pathfinder-Triad of System 2, the only open-source decoder systems reported to strictly beat Lange's GNN with non-overlapping confidence intervals.

All code, trained checkpoints, benchmark scripts, and raw evaluation data are at **https://github.com/bledden/pathfinder**.
...
**Pathfinder** — a direction-specific 3D CNN trained by a single recipe (init from a 3-parameter Table-1 checkpoint, 40K fine-tune steps at Lange's 4-parameter noise model, Muon + AdamW, same script at every code distance). One canonical checkpoint per distance; the checkpoint at d=7 is the one benchmarked in every Pathfinder row of §5.11 and §5.12. At d=7 p=0.007 (60K shots, 4-parameter noise), Pathfinder achieves LER 3.09%, statistically tied with PyMatching (3.34%, overlapping 95% CIs) and ~5% relative above Lange (2.94%).

**Pathfinder-Triad** — a three-way majority-vote ensemble of (Pathfinder, Lange et al. [14], PyMatching [2]). Each shot is decoded by all three; the ensemble prediction is the elementwise majority of the three binary outputs. No additional training. The Triad's LER depends on which Pathfinder voter is plugged in (canonical fine-tune, Wide-Long, or PFWL3S 3-seed-avg); the lowest measured Triad LER at d=7 p=0.007 is **2.384%** (with the PFWL3S 3-seed-avg voter; §5.13, 100K shots) — non-overlapping 95% Wilson CI [2.30, 2.50] vs Lange's [2.85, 3.06]. With the canonical fine-tune voter (the original §5.12 system, 100K shots), Pathfinder-Triad achieves LER **2.454%** with non-overlapping CI [2.36, 2.55]. The relative LER improvement over Lange alone is therefore between 17.0% (canonical voter) and 19.4% (PFWL3S voter); the strict-CI claim holds for both. Subsequent sections quote **2.384%** as the headline Triad number when emphasizing the lowest-LER claim and **2.454%** when contrasting against the original §5.12 result.

The paper's distinct contributions are:

1. **Pathfinder-Triad strictly beats Lange alone at d=7 operational noise rates**, with statistically significant CI separation at p ∈ {0.007, 0.010, 0.015} for the PFWL3S-voter Triad and at p ∈ {0.007, 0.010} for the canonical-voter Triad (§5.12, §5.13). With the PFWL3S-H256-d9 voter the same Triad strict-CI win extends to d=9 p ∈ {0.007, 0.010} (§6.3), giving 5 (d, p) strict-CI Triad wins across two distances. This is, in the measurements reported here, the lowest open-source LER at d=7 and d=9 operational noise rates under Lange's 4-parameter noise model.
2. **Canonical Pathfinder + Triton is the only open-source decoder tested that sustains the d=7 7-μs cycle-time budget** (6.12 μs/syn at B=1024 on H200; Table 3d), while still beating PyMatching under 3-parameter noise (Table 1). This claim is for **single-seed canonical Pathfinder (H=256, 500K params)** with the Triton kernel — *not* PFWL3S (3-seed-avg, ~24 μs/syn extrapolated, off-budget) and *not* Pathfinder-Triad (~72 μs/syn Lange-bounded, off-budget). The canonical-Pathfinder + Triton system loses to Lange individually by ~5% relative at d=7 p=0.007 (LER 3.34% vs 2.94%); the trade is real-time-budget compliance versus 17–19% lower LER from the off-budget Triad. Lange's GNN on the same H200 is 12× slower per syndrome than canonical Pathfinder + Triton (71.67 vs 6.12 μs/syn); PyMatching on a single Apple M4 core is 9.65 μs/syn at p=0.007, also above the budget.
...
**Figure 1.** *Pathfinder-Triad beats every individual decoder at d=7 operational rates.* Logical error rate as a function of physical error rate p at d=7, log–log scale. The pale gold band marks the operational regime p ∈ {0.005, …, 0.015} where the Pathfinder-Triad system (purple, hero) achieves strict-CI dominance over Lange's GNN (red), PFWL3S alone (blue), and PyMatching (grey). At the headline operational rate p=0.007 the Triad reaches 2.38% LER versus Lange's 2.96% — a 262 bp gap that does not overlap on 95% Wilson CIs at 100K shots. Data from `bench/results/h200_main/tierC1/ensemble_pfwl3s_full.json`.

### 5.1 Main Results: Rotated Surface Code

Table 1 presents the definitive evaluation: all decoders on the rotated surface code at distances d=3, 5, 7 across 8 noise rates, with 100,000 shots per data point. Pathfinder wins or ties PyMatching at every one of the 24 evaluation points — 22 wins plus 2 exact ties at p=0.0005, d=5 and d=7 where both decoders achieve zero observed errors in 100,000 shots. Twelve of the 24 points show non-overlapping 95% Wilson confidence intervals for the two decoders; the remaining 12 are either exact ties (2) or points where the noise rate is low enough that small-number statistics yield overlapping intervals (10, one of which — d=7 p=0.003 — overlaps by only 0.001 pp and is effectively at the boundary). See the footnote below Table 1 for the statistical-significance breakdown.
...
Bold indicates the lower (better) LER. Pathfinder wins or ties at every one of the 24 evaluation points. **Statistical significance:** computing 95% Wilson confidence intervals for each entry (N=100,000), 12 of 24 points show non-overlapping CIs between Pathfinder and PyMatching; the remaining 12 include two exact ties (both decoders at 0 errors, p=0.0005 at d=5 and d=7) and ten points where the low noise rate (typically p ≤ 0.003) produces so few decoding failures that CIs overlap (the closest, d=7 p=0.003, overlaps by only 0.001 pp). The non-overlapping-CI wins span every tested distance and concentrate at p ≥ 0.005 where the decoding regime is most relevant for real hardware; Pathfinder is never observed to lose.

**Model-selection disclosure.** Pathfinder's Table 1 entries use the per-noise-rate specialization of §4.5 — at each noise rate, the best noise-rate-targeted checkpoint/ensemble. A *single fixed* d=7 checkpoint (`d7_final`) does **not** beat PyMatching at the higher operational rates: §5.6 Table 5 reports it at 1.67% vs PM 1.50% (p=0.007) and 6.63% vs 5.22% (p=0.010). The Table 1 wins at p ≥ 0.007 therefore reflect noise-rate-matched model selection, not one deployable checkpoint. (This per-rate selection applies only to the 3-parameter Table 1; the §5.11/§5.12 4-parameter head-to-heads against Lange use a single canonical checkpoint per distance, and the §5.13 PFWL3S results use a fixed 3-seed ensemble — neither is per-rate-selected.)

**A note on the chosen statistical test.** Throughout this paper, "stat-sig" / "strict-CI win" means *non-overlapping 95% Wilson confidence intervals on the two decoders' per-shot error proportions, evaluated on independent shots*. This is a deliberately conservative test: it is *more conservative* than a 2-sample test of difference in proportions (e.g., normal approximation, Fisher exact, or a paired test on per-shot agreement). The non-overlap criterion can fail to reject equality even when a paired test would reject. Adopting non-overlap as the threshold therefore means the paper's stat-sig win counts are lower bounds — a paired-test reanalysis would generically find more significant differences than reported. I chose non-overlap for its simplicity, audit-ability from the published per-point JSONs (anyone can recompute Wilson CIs without reanalysis machinery), and conservatism with respect to the headline claims. **Two conventions used throughout.** (i) *Gap units:* I report LER gaps in three explicitly-distinct forms — the **point-estimate gap** (difference of point LERs), the **CI-edge separation** (gap between the nearer 95%-Wilson-CI edges; positive ⇒ non-overlapping ⇒ a strict-CI win), and the **relative reduction** (%); where a bare "bp" appears it denotes 0.001 percentage-points of LER (i.e. 10⁻⁵ in fraction, one-tenth of a financial basis point) and always refers to the **CI-edge separation** unless labelled "point-estimate gap." (ii) *Paired test:* because both decoders decode the same shots, I additionally report **McNemar's paired test** on the per-shot agreement table alongside the conservative marginal-CI criterion; McNemar is the more powerful and standard choice and is reported with each headline comparison.
...
![Pathfinder wins or ties PyMatching at 24/24 (d, p) points — 3-parameter circuit-level noise](../figures/fig02_3param_multid.png){.fig}
...
Pathfinder's inference latency was measured on two GPUs: the AMD MI300X used for training, and the NVIDIA H200 SXM used by Gu et al. [8] — providing an apples-to-apples comparison on equivalent hardware. The H200 latency numbers below use `torch.compile(mode="max-autotune")` and FP16 (CUDA 12.4). The **custom Triton kernel** path (the headline 6.12 μs/syn budget figure) bypasses Inductor codegen and is PyTorch-version-independent (§5.3 "Reproducibility (Triton kernel)"). The **Inductor** path is version-sensitive: the committed head-to-head artifact (`h200_final_benchmark.json`) carries a torch-2.6.0 environment tag, while the per-configuration sweeps isolate the 2.4.1-vs-2.6.0 Inductor difference (see "Version sensitivity" below) — the head-to-head was measured on torch 2.6.0 (the committed artifact) and **cannot be reproduced on torch 2.4.1** (its `max-autotune` path fails to compile the custom kernel), so reproduce on the 2.6.0 stack (reproduction note at the end of this section; Appendix A.3).

**Version sensitivity (important for reproduction).** This latency is sensitive to **both** the PyTorch/Inductor version **and** the `torch.compile` mode, so the numbers in this section span more than one stack — a clean single-stack re-measurement of the full latency table is a pre-submission item. Two measured facts: (i) for the *plain* `torch.compile` (reduce-overhead) Inductor path at d=7, B=1024, FP16, the latency is **6.4 μs/syn on PyTorch 2.4.1** (`bench/results/h200_latency_results.json`) vs **15.5 μs/syn on PyTorch 2.6.0** (`bench/results/h200_latency_torch26.json`) — an Inductor codegen regression in that mode; (ii) the **headline Triton-vs-Inductor head-to-head (Table 3b/3d, 6.12 / 7.86 μs/syn, `max-autotune`)** was measured on **PyTorch 2.6.0** (`bench/results/h200_final_benchmark.json`, which carries that env tag). Note the two facts use different compile modes, so they are not directly comparable, and on torch 2.4.1 the `max-autotune` path **fails to compile the custom Triton kernel** (an Inductor/`free_unbacked_symbols` error on the kernel's output dict) — so the head-to-head cannot be reproduced on 2.4.1 and the committed artifact is the 2.6.0 one. The **custom Triton kernel's correctness** (its numerical-equivalence gate, §5.3) is version-independent; only the `torch.compile`-mediated *latency* is stack-sensitive.
...
**Reproduction note.** The two head-to-head figures (custom kernel **6.12** vs Inductor **7.86** μs/syn at d=7, B=1024) are the `full_H256_triton` / `full_H256` configs of `bench/results/h200_final_benchmark.json`, produced on a **PyTorch 2.6.0** stack with `max-autotune`. They **cannot be reproduced on torch 2.4.1**: there, `torch.compile(max-autotune)` fails to compile the custom Triton kernel (an Inductor `free_unbacked_symbols` error on the kernel's output dict), and the plain-Inductor `full_H256` measures 8.84 μs/syn rather than 7.86. A clean single-stack (torch 2.6.0) re-measurement of the whole latency table, committed alongside `h200_final_benchmark.json`, is the recommended pre-submission step.
...
![Pathfinder and Lange fail on partially-overlapping syndromes (both wrong on 1.8% of all shots; 43% of their combined failures)](../figures/fig05_failure_overlap.png){.fig}

**Figure 5.** *Per-shot decoder agreement at d=7, p=0.007 (60K shots; data from `bench/results/h200_main/phase2/ensemble_results_final.json`).* Venn diagram of the failure sets: Pathfinder is wrong on 1,854 shots, Lange is wrong on 1,764 shots, both are wrong on only 1,083 shots. The two decoders co-fail on 1,083 of their 2,535 combined failures (43%) but each independently catches ~700 shots the other misses — i.e. the failure sets are *partially* overlapping, not disjoint, and the decoders disagree on which shots fail ~57% of the time. (The stronger 0.01%-overlap figure above is the Pathfinder-vs-PyMatching pair at d=5, not this Pathfinder-vs-Lange d=7 pair.) This partial independence is the structural reason the §5.12 three-way majority vote (Pathfinder-Triad) achieves lower LER than any individual decoder — partially-independent error coverage, not architectural similarity, is what the ensemble exploits.
...
(PF values here are higher than Table 1's d=7 PF row because Table 1 uses the noise-rate-specialized ensemble described in §4.5; this §5.6 table uses the single `d7_final` checkpoint to make the PF + PM ensemble analysis interpretable. The PM values here match Table 1's d=7 PM column within shot variance.)
...
Pathfinder converges in approximately 82 million training samples (80K steps × batch 1024) for d=5–7. Gu et al. [8] report using 266 million samples (80K steps × batch 3,328), suggesting that the compressed curriculum and Muon optimizer achieve 3.2× better sample efficiency.
...
*The H=192-distilled LER (2.176%) is a 100K-shot canonical re-evaluation; an earlier draft reported 2.035%, which did not reproduce (the corrected value does not change the row's verdict — it still does not beat PM). †Its 5.05 μs/syn latency lacks a committed measurement JSON and could not be re-measured here: torch-2.4.1's Inductor `max-autotune` path fails to compile the custom Triton kernel (see §5.3 "Version sensitivity"); it needs a re-measurement on the torch-2.6.0 stack the other latency numbers use.*
...
| 7 | 0.007 | 3.090% | **2.940%** | 3.343% | overlap |
| 7 | 0.010 | 11.132% | 10.822% | **10.300%** | overlap |

Bold = lowest LER in the row. Under this matched-noise comparison **Lange's GNN has the lowest LER of the two neural decoders at every tested point** (at d=7 p=0.010 PyMatching's LER edges below both neural decoders). Pathfinder (canonical) and Lange each beat PyMatching at 11 of 12 points — both lose to PM only at d=7 p=0.010. The Pathfinder–Lange point-estimate gap ranges from ~2% relative at d=3 to ~15% relative at d=5 p=0.007, and is ~5% at d=7 p=0.007; by the non-overlapping-CI test at 60K shots the gap is significant only at d=5 p ∈ {0.005, 0.007, 0.010} (the d=7 p=0.007 and p=0.010 gaps are within overlapping CIs — a point-estimate trail, not a significant loss). The full 21-point sweep extending to p ∈ {0.001, 0.002, 0.015} is in `bench/results/h200_lange_headtohead_{low,high}_p.json`; the qualitative conclusion is the same.

**Interpretation — Lange's architecture is stronger individually.** The canonical Pathfinder fine-tune recipe closes most of the out-of-distribution gap (Pathfinder goes from 4.01% OOD → 3.09% in-distribution at d=7 p=0.007), but not all of it. The residual gap — statistically significant at d=5 (~15% relative at p=0.007, non-overlapping CIs) and a within-noise ~5% point-estimate trail at d=7 p=0.007 (overlapping CIs at 60K shots) — is, in my best reading, a real architectural advantage of Lange's GNN at d=5 on this task: graph-of-defects message passing with KNN edges appears to be a more inductively-appropriate representation than 3D lattice-aware convolution for this specific noise model and decoding objective. The 1.36M-parameter GNN (Lange) vs. 500K-parameter CNN (Pathfinder) parameter gap is not the explanation: Pathfinder with 4.36M parameters and modern attention primitives (the hybrid of §5.14) is *worse*, not better.

**Paired-test reanalysis (McNemar).** The non-overlapping-CI criterion used throughout is a *marginal* test; since both decoders see the *same* syndrome samples, the more powerful and appropriate test is McNemar's on the per-shot agreement table. Applied to the canonical-Pathfinder-vs-Lange comparison (60K shots; per-shot decomposition in `bench/results/h200_main/phase2/`), it sharpens the verdict: at d=7 p=0.007 the discordant counts are 771 (Lange right / PF wrong) vs 681 (PF right / Lange wrong) → χ²=5.46, **p=0.020**; at d=7 p=0.010, 2857 vs 2671 → χ²=6.19, **p=0.013**. So under the paired test, canonical single-checkpoint Pathfinder **significantly trails Lange at d=7** — the marginal-CI test had called these points overlapping/non-significant, because it is the weaker test. This does not weaken the paper; it *strengthens the motivation* for the PFWL3S recipe (§5.13) and the Triad (§5.12): canonical Pathfinder is genuinely behind Lange as an individual decoder at d=7, and it takes the wider/longer/3-seed PFWL3S recipe and the three-way ensemble to overturn that. The strict-CI wins reported for PFWL3S and the Triad are therefore stated under the *conservative* marginal test; the corresponding McNemar tests (reported with each headline claim) are at least as significant.

**Negative result — from-scratch training at 4-parameter noise is unreliable.** I also attempted training Pathfinder from scratch at 4-parameter noise (`train_fixed_noise.py`, same curriculum as Table 1 but with the extra noise parameter). At d=5 it converged to 11.7% — worse than even the OOD Table-1 checkpoint (2.94%); at d=7 it failed catastrophically (LER stuck at ~40% throughout the run). Training logs and failed checkpoints are preserved under `bench/results/h200_main/` and `h200_main/phase5/`. The lesson is that the Pathfinder recipe at this scale benefits from a warm 3-parameter-noise init even when the downstream target is 4-parameter noise; a from-scratch 4-parameter run is not reliable in an 80K-step budget. This is relevant to the d=9 extension attempt discussed in §6.3.
...
| **0.007** | **2.492%** [2.397, 2.591] | 2.956% [2.853, 3.063] | **2.739%** [2.640, 2.842] | 2.384% | **2.326%** | **49 bp (PF upper 2.591, Lange-FT lower 2.640)** | **STRICT WIN preserved — PFWL3S still beats fine-tuned Lange** |
| **0.010** | **9.173%** [8.996, 9.354] | 10.764% [10.573, 10.958] | **10.088%** [9.903, 10.276] | 8.689% | **8.547%** | **55 bp (PF upper 9.354, Lange-FT lower 9.903)** | **STRICT WIN preserved** |
| **0.015** | **27.328%** [27.05, 27.61] | 30.200% [29.92, 30.49] | **28.153%** [27.88, 28.43] | 25.872% | **25.508%** | **27 bp (PF upper 27.605, Lange-FT lower 27.875)** | **STRICT WIN preserved** |

**Findings.** (a) **Fine-tuning Lange at p=0.007 does improve Lange** at every operational rate where it was OOD (relative LER reductions of 7% at p=0.007, 6% at p=0.010, 7% at p=0.015), closing the OOD gap but not the architectural gap. (b) **PFWL3S still strictly beats fine-tuned Lange** at all three d=7 operational rates with non-overlapping 95% Wilson CIs, with absolute CI gaps shrinking from 262/1219/2311 bp (vs published Lange, §5.13 Table) to 49/549/270 bp (vs fine-tuned Lange). The fine-tuning closes most of the headline LER gap but **never inverts the direction of the comparison** (this is the marginal-CI test against the *single* fine-tuned Lange; the paired-McNemar control against a *full-recipe 3-seed Lange ensemble* in C3 below shows the win is robust at p=0.007/0.010 but a tie at p=0.015). (c) **Pathfinder-Triad with the fine-tuned Lange voter is slightly *better*** than Triad with published Lange at every operational rate (e.g., d=7 p=0.007: 2.326% vs 2.384%, a 58 bp improvement) — fine-tuning Lange gives the Triad's Lange voter a better signal, and the majority vote inherits the improvement. (d) **At p=0.005 fine-tuning does not help Lange** (Lpub 0.727% vs Lft 0.717%, within noise) because p=0.005 is already inside Lange's published training distribution; the OOD-vs-IID concern only applies at p > 0.005.
...
| d=7 rate | PFWL3S | Lange-FT 3-seed (full recipe) | marginal-CI | McNemar (paired) | Holm (3-rate family) |
|---|---:|---:|:---:|:---:|:---:|
| p=0.007 | **2.533%** | 2.652% | PFWL3S strict | χ²=9.15, **p=0.0025** | **significant** |
| p=0.010 | **9.140%** | 9.538% | PFWL3S strict | χ²=27.6, **p<10⁻⁴** | **significant** |
| p=0.015 | 27.304% | 27.542% | PFWL3S strict | χ²=2.84, **p=0.092** | **not significant** |

**The honest reading.** Against the toughest control — a full-recipe 3-seed Lange ensemble — PFWL3S's win is **robust at p=0.007 and p=0.010**: it holds under *both* the marginal-CI test and the more-powerful paired McNemar test, and survives Holm correction across the three operational rates (the p=0.010 win survives even Bonferroni over all 24 (d, p) comparisons). At **p=0.015 the win does *not* survive the paired test** — the full-recipe ensemble closes to within McNemar noise (p=0.092), and PyMatching (27.269%) also matches PFWL3S (27.304%) at that high rate. So the marginal-CI "strict win" at p=0.015 is an artifact of the marginal test treating the two decoders as independent; the paired test, which is correct here, does not support it. **The defensible C3 claim is therefore: PFWL3S strictly and significantly beats a full-recipe 3-seed Lange ensemble at p=0.007 and p=0.010 (most robustly at p=0.010), and ties it at p=0.015.** This is the matched-ensemble, paired-test, multiplicity-corrected control; the win is not an ensemble-size or training-budget artifact at the two lower operational rates, and is honestly not robust at the highest. The published-Lange control in the same run reproduces at 3.027% (vs the paper's 2.956%), validating the harness; PFWL3S in this control reads 2.533% (a fresh, independent 100K-shot sample), consistent with the canonical 2.492% headline within shot-noise. Data: `coda_experiments/lange_3seed_eval.json`; eval: `coda_experiments/eval_lange_3seed.py`.
...
**Figure 6.** *PFWL3S vs published-Lange vs fine-tuned-Lange at d=7, operational noise rates (100K shots/point).* Fine-tuning Lange at p=0.007 closes most of the OOD gap (2.96% → 2.74%, ~7% relative improvement at the headline rate) but does **not** invert the direction of the strict-CI comparison: PFWL3S still strictly beats fine-tuned Lange at every d ≥ 7 operational rate. The shaded callout marks the 49 bp CI gap at p=0.007; analogous strict-CI wins hold at p=0.010 and p=0.015. The architectural reversal is therefore a controlled finding, not an artifact of Lange's published training distribution.
...
**Figure 7.** *Strict-CI dominance grid across all (d, p) operational points.* Rows are (d, p) combinations from d=3 to d=9 at p ∈ {0.005, 0.007, 0.010, 0.015}; columns are the four head-to-head comparisons that define this paper's headline claims. Green cells show row-decoder strict-CI wins (95% Wilson non-overlap) with the CI-gap magnitude in basis points; purple cells show strict losses; pale-grey cells show statistical ties. The right two columns (Triad vs Lange-pub, Triad vs PyMatching) collect the §5.12/§6.3 ensemble wins; the left two columns (PFWL3S vs Lange-pub, PFWL3S vs Lange-FT) collect the §5.13/C2 individual-decoder wins. The pattern is consistent: wins concentrate at d ≥ 7 and at operational rates p ≥ 0.007, exactly where the three decoders' independent failure modes give the ensemble the most coverage to recover.
...
|---|---:|---|---:|---:|---:|
| PyMatching | 3.343% | — | — | — | — |
| Canonical Pathfinder (single ckpt, 60K) | 3.09% | — | −0.13 (PF *trails*; McNemar p=0.02) | overlap | — |
...
*Conventions (see §5.1 note): "point-estimate gap" = difference of point LERs; "CI-edge separation" = gap between nearer 95%-Wilson edges (>0 ⇒ strict-CI win); "rel. ↓" = (Lange−X)/Lange. Where the prose writes "bp" it means 0.001 pp (so 0.262 pp = "262 bp"). The controlled comparisons against fine-tuned Lange (single, C2) and the full-recipe 3-seed Lange ensemble (C3, with McNemar) are in §5.11; the multi-distance Triad-vs-Lange wins are in §5.12 / §6.3.*
...
| d=7 | **Pathfinder-Wide-Long-3seed (H=384, 3-seed avg of 160K-step ckpts)** | **3.27M total** | **2.492%** ★★ | [2.397, 2.591] | **2.384%** | **STRICT WIN — non-overlapping CIs, 262 bp gap** |
| d=5 | **Pathfinder-Wide-Long-3seed (H=384, 3-seed avg of 160K-step ckpts)** | **~2.55M total (3 × 850K)** | 2.539% | [2.443, 2.638] | **2.476%** | tied at p=0.007; **strict win at p=0.015** (PF 17.21% vs Lange 17.90%, 222 bp gap) |
...
| p=0.005 | **0.657%** [0.609, 0.709] | 0.727% [0.676, 0.782] | **0.621%** | overlap by 3 bp (soft win) |
| p=0.007 | **2.492%** [2.397, 2.591] | 2.956% [2.853, 3.063] | **2.384%** | **NON-OVERLAP, 262 bp gap, 15.7% relative** |
| p=0.010 | **9.173%** [8.996, 9.354] | 10.764% [10.573, 10.958] | **8.689%** | **NON-OVERLAP, 1219 bp gap, 14.8% relative** |
| p=0.015 | **27.328%** [27.05, 27.61] | 30.200% [29.92, 30.49] | **25.872%** | **NON-OVERLAP, 2311 bp gap, 9.5% relative** |

The single-seed bottleneck identified by Pathfinder-Wide-XLong (240K-step single-seed plateau; §5.13 above) was the binding constraint — averaging predictions across three different random seeds tightens the effective LER by ~10–15 percentage points relative to any single seed at the matched noise. Pathfinder-Wide-Long-3seed therefore delivers the paper's **strictest open-source LER claim**: a CNN-vs-GNN architectural reversal where Pathfinder's direction-specific 3D convolution + Muon-distillation recipe + multi-seed averaging exceeds Lange's published GNN by 9–16% relative LER with non-overlapping confidence intervals at three of three tested operational d=7 noise rates and (extension below) one of four tested d=5 noise rates. Inference latency cost: 3× per-shot CNN forward pass (still much smaller than Lange's per-shot GNN forward); ckpts are at `bench/results/h200_main/tierC1/pathfinder_wide_long_d7{,_seed1,_seed2}/best_model.pt` and the eval script that averages logits is `bench/results/h200_main/tierC1/multiseed_eval.log`. Pathfinder-Triad with the 3-seed-avg voter at the headline d=7 p=0.007 point achieves LER **2.384%**, the lowest open-source LER reported in this paper.

**Extending PFWL3S to d=5 and d=3 — multi-seed Wide-Long at every distance.** I subsequently trained three independent random-seed Wide-Long ckpts at each of d=5 and d=3 using the same recipe as d=7 (H=384, 160K steps, distill from Lange teacher at single-noise p=0.007, `muon_lr=0.005`) and re-ran the 8-noise sweep at 100K shots with 3-seed-avg ensembling at every distance. Data: `bench/results/h200_main/tierC1/ensemble_pfwl3s_v2.json`; eval log: `eval_pfwl3s_v2.log`.
...
| Reference impl, 3-seed avg | 9.17 ms (= 3.0×) | **61.2 μs/syn** (= 3× single) | — |
...
**Important methodological note on round count.** Every Pathfinder checkpoint in this paper — single-seed and PFWL3S 3/5-seed alike — was trained with `rounds = distance` (the canonical R=d Stim memory-experiment configuration). The 3D-CNN's direction-specific temporal weights (`w_tp`, `w_tm` in `DirectionalConv3d`) learn correlations across the T = R+1 detector timepoints at training. **A first IBM submission attempted at d=5 R=1 produced a misleading PFWL3S=33.07% / PM=16.53% result that is reported in the earlier-draft Table 13 of this section; that result is now retracted as a train/test rounds-mismatch artifact, not an OOD-noise finding.** Re-decoding with the proper PFWL3S 5-seed at `R=d=5` (where the chip's noise structure is the only remaining train/test mismatch) gives the matched-distribution numbers in Table 13a below. The d=3 R=3 result is similarly free of the rounds-mismatch artifact.

**Table 13a: IBM Heron r2 (`ibm_fez`) — PM vs PFWL3S vs Lange vs Pathfinder-Triad at matched rounds (10K shots/point)**
...
This is reported as a **provisional positive result: a statistical tie with PyMatching at d=3 r=3** (the d=5 r=5 point is past-threshold and near-random, so it does not discriminate decoder quality), with the residual single-p OOD gap and the engineering path that closes it (calibrated-noise training) explicitly documented in Table 13a rows 2–3. It is meaningfully stronger evidence for the §5.11 / §5.13 family than the §5.15 Willow result on its own and supersedes the §5.15 footnote that PFWL3S "fails on real hardware" — that statement was true under input-adapter mismatch and circuit-format mismatch, and is *not* true under matched topology and matched rounds at d=3.
...
**Table 13b: IBM Heron r2 soft (analog-IQ) vs hard readout, 20K kerneled-IQ shots/point.** Two independent IQ→probability calibrations are shown (v1: fitted-weight 2-Gaussian posterior; v2: training-matched equal-prior posterior) to confirm the result is calibration-robust.
...
**What this establishes.** Soft/analog readout does **not** let PFWL3S beat PyMatching on real `ibm_fez` at either tested distance — a negative result, but a *trustworthy* one: the soft pipeline is correctness-gated, and the decode-path bug that initially faked a negative was found and fixed. The infrastructure (kerneled-IQ collection, the 2-Gaussian soft pipeline, soft-aware training, the float-soft decode) is reusable. **Whether soft readout would help on a noisier-readout, sub-threshold device is untested speculation** — this paper ran no synthetic positive control (injecting graded readout in simulation where soft *provably* should help, then showing soft beats hard), so the null here establishes only that soft does not help on `ibm_fez`, not that the pipeline can extract soft gain where it exists. That positive control is the cleanest next test. Data: `coda_experiments/ibm_soft_v2_{d3r3,d5r5}_eval.json`; pipeline + calibration: `coda_experiments/{soft_info.py, soft_info_v2.py, soft_detmap.py}`; training: `train/{train_soft.py, data_soft.py}`.
...
**Table 14: Hybrid CNN+GNN 3-seed avg vs PFWL3S 3-seed avg, d=7, matched 4-parameter noise, 100K shots/p**
...
2. **Establishes a clean baseline for the architectural-fusion question.** Anyone publishing a "CNN+GNN beats CNN-alone" claim in the surface-code decoder space now needs to beat the 8/8-overlap row in Table 14 with the same training budget; this is a falsifiable claim, not "we didn't try it."
...
| 9 | 0.007 | 3.310% [3.201, 3.423] | **2.623%** | 3.155% | **2.277%** [2.184, 2.374] | Lange strict-wins | **STRICT WIN — Triad beats Lange, 154 bp gap, 13.2% relative** |
| 9 | 0.010 | 15.061% [14.841, 15.284] | **13.085%** | 12.227% | **10.852%** [10.660, 11.045] | Lange strict-wins | **STRICT WIN — Triad beats Lange, 1831 bp gap, 17.1% relative** |
...
**Figure 11.** *d=9 decoder comparison at operational noise rates p ∈ {0.005, 0.007, 0.010, 0.015}, log-y.* Four grouped bars per noise rate: PyMatching (grey), Lange GNN (red), PFWL3S-H256-d9 (blue), Pathfinder-Triad (purple). Error bars are 95% Wilson CIs at 100K shots/point. PFWL3S-H256-d9 loses to Lange individually at every operational rate — the architectural-reversal of §5.13 does not extend to d=9 at H=256. But the **Pathfinder-Triad still strictly beats Lange** at p=0.007 (154 bp gap, 13.2% relative) and p=0.010 (1831 bp, 17.1% relative), confirming the ensemble's value-add is *structural* (independent failure-mode coverage) rather than tied to a competitive individual PF voter.

**Noise models.** Evaluation is primarily on circuit-level depolarizing noise and (for generalization) phenomenological noise. Two real-hardware validation runs are now folded in: (a) **Google Willow Sycamore d=7 r=13 traces (§5.15)** — PyMatching ran cleanly at 4.006% LER consistent with the Willow paper, while PFWL3S produced ~46% (near-random) predictions because its trained weights expect a different detector-tensor format (R=d=7 standard-Stim 4-parameter rather than R=13 SI1000 with compound-detector L=6/9/15 elements), an *adapter-mismatch* artifact rather than a noise-OOD finding; (b) **IBM Heron r2 `ibm_fez` d=3 r=3 and d=5 r=5 (§5.15.1)** — controlled-circuit submission where I removed both the adapter-mismatch and rounds-mismatch confounds: **PFWL3S ties PyMatching at d=3 r=3 (28.98% vs 28.49%, CIs overlap)** and at d=5 r=5 both decoders are near-random in a chip-past-threshold regime (47.27% vs 45.68%, observable-flip 0.49), so that point does **not** discriminate decoder quality — the calibrated-vs-single-p delta there (0.41 pp) is within the chip's ~1.6 pp cross-epoch PM drift and is therefore **not** attributed to training. The d=3 r=3 tie is the decoder-discriminating real-hardware result. The recipe used at d=5 is in `train/data_calibrated.py` + `train/train_calibrated.py`; the recipe is in `train/data_calibrated.py` + `train/train_calibrated.py`. A full comparison to AlphaQubit [5] on experimental Sycamore data is still out of scope (AlphaQubit's weights are not public).
...
- Python 3.11
- PyTorch ≥ 2.4 (training); **torch 2.6.0 for the §5.3 head-to-head latency** (the committed `h200_final_benchmark.json` stack — torch 2.4.1 fails to compile the `max-autotune` Triton path; separately, the plain reduce-overhead Inductor probe regresses on 2.6, §5.3 "Version sensitivity")
...
# PyTorch 2.6.0 + its bundled Triton  (head-to-head stack; torch 2.4.1 can't compile the max-autotune Triton path — see §5.3 "Version sensitivity")
