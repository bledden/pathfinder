...
**Findings.** (a) **Fine-tuning Lange at p=0.007 does improve Lange** at every operational rate where it was OOD (relative LER reductions of 7% at p=0.007, 6% at p=0.010, 7% at p=0.015), closing the OOD gap but not the architectural gap. (b) **PFWL3S still strictly beats fine-tuned Lange** at all three d=7 operational rates with non-overlapping 95% Wilson CIs, with absolute CI gaps shrinking from 262/1219/2311 bp (vs published Lange, §5.13 Table) to 49/549/270 bp (vs fine-tuned Lange). The fine-tuning closes most of the headline LER gap but **never inverts the direction of the comparison**. (c) **Pathfinder-Triad with the fine-tuned Lange voter is slightly *better*** than Triad with published Lange at every operational rate (e.g., d=7 p=0.007: 2.326% vs 2.384%, a 58 bp improvement) — fine-tuning Lange gives the Triad's Lange voter a better signal, and the majority vote inherits the improvement. (d) **At p=0.005 fine-tuning does not help Lange** (Lpub 0.727% vs Lft 0.717%, within noise) because p=0.005 is already inside Lange's published training distribution; the OOD-vs-IID concern only applies at p > 0.005.

**Controlled fairness check C3 — does the win survive when Lange is *also* a 3-seed ensemble?** The headline PFWL3S is a 3-seed logit-mean ensemble, whereas C2 compares against a *single* fine-tuned Lange GNN — leaving open whether PFWL3S's margin is merely an ensemble-size effect. To close this, I fine-tuned **three** independent Lange seeds at p=0.007 and ensembled them by logit-mean (matched to PFWL3S's averaging), then re-ran the d=7 head-to-head at 100K shots. **PFWL3S strictly beats the 3-seed fine-tuned-Lange ensemble at all three operational rates with non-overlapping 95% Wilson CIs:** p=0.007 (2.533% vs 2.838%), p=0.010 (9.140% vs 10.084%), p=0.015 (27.304% vs 28.866%). The architectural reversal is therefore **not** an ensemble-vs-single-model artifact. (Caveat: these three Lange seeds used a lighter fine-tune recipe — 200K samples/epoch × 14 epochs vs C2's single seed at 2M × 30 — so the ensemble at 2.838% is marginally *weaker* than C2's single full-recipe FT-Lange at 2.739%; PFWL3S at 2.533% strict-beats **both**, and closing the ~10% relative gap from the ensemble down to PFWL3S would require far more than 3-seed averaging delivers. The published-Lange control in the same run reproduces at 3.027% vs the paper's 2.956%, validating the harness. At p=0.015 PyMatching, 27.27%, edges PFWL3S, 27.30% — at this high rate PM and PFWL3S are tied while both beat the Lange ensemble, so the PFWL3S-beats-Lange claim is unaffected.) Data: `coda_experiments/lange_3seed_eval.json`; eval: `coda_experiments/eval_lange_3seed.py`.
...
**What this establishes.** Soft/analog readout does **not** let PFWL3S beat PyMatching on real `ibm_fez` at either tested distance — a negative result, but a *trustworthy* one: the soft pipeline is correctness-gated, and the decode-path bug that initially faked a negative was found and fixed. The infrastructure (kerneled-IQ collection, the 2-Gaussian soft pipeline, soft-aware training, the float-soft decode) is reusable and would be expected to pay off on a noisier-readout, sub-threshold device — the appropriate hardware to revisit the AlphaQubit-style soft-decoding advantage. Data: `coda_experiments/ibm_soft_v2_{d3r3,d5r5}_eval.json`; pipeline + calibration: `coda_experiments/{soft_info.py, soft_info_v2.py, soft_detmap.py}`; training: `train/{train_soft.py, data_soft.py}`.
