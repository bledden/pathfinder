"""
Distill Lange's GNN into Pathfinder architecture.

Insight: Lange is pre-trained and beats us. We can use Lange as an oracle teacher,
train our model to match both (a) true labels and (b) Lange's soft predictions.
  Loss = α * BCE(student_logit, label) + β * T² * KL(softmax(student/T), softmax(teacher/T))

Result at best: matches Lange's accuracy with our architecture's speed.
At worst: matches whichever teacher/labels give clearer signal.
"""
...
class LangeTeacher:
    """Wrapper around Lange's GNN for training-time inference."""
    def __init__(self, d, d_t):
        ...
        self.m_nearest = 10
        self.power = 2
        # Cached per-circuit data
        self._cache = {}

    def init_for_circuit(self, circuit, p):
        """Cache detector coords and syndrome mask for this noise rate."""
    ...
    def teacher_logits(self, det_events, det_coords, syn_mask):
        """Returns teacher logits (raw, not sigmoid) for the batch.
        Trivial syndromes get 0 logit (equal prior = 50/50, teacher uncertain)."""
        B = det_events.shape[0]
        mask = np.repeat(syn_mask[None, ...], B, 0)
        s3d = np.zeros_like(mask)
        s3d[:, det_coords[:, 1], det_coords[:, 0], det_coords[:, 2]] = det_events
        ...
        with torch.no_grad():
            out = self.model(x, ei, batch, ea)  # [num_graphs, 1]
        # Map graph outputs back to per-batch results
...
def main():
    ...
    a.add_argument("--batch", type=int, default=256)  # smaller for teacher overhead
    a.add_argument("--noise_rate", type=float, default=0.003)  # where Lange is strong
    ...
    a.add_argument("--ckpt", type=str, default="/workspace/pathfinder/train/checkpoints/distill_d5")
    a.add_argument("--init", type=str, default=None, help="Optional init checkpoint for student")
    a.add_argument("--eval_interval", type=int, default=5000)
    a.add_argument("--log_interval", type=int, default=500)
    args = a.parse_args()
    os.makedirs(args.ckpt, exist_ok=True)

    # Student (Pathfinder architecture, same as v1)
    ...
    print(f"Teacher (Lange GNN d={args.distance} d_t={args.distance}) loaded and frozen", flush=True)

    # Dataset at Lange's training range. Single noise point for now.
    ...
    # Pre-compute Lange's per-circuit cache
    dc, syn_mask = teacher.init_for_circuit(ds.circuit, args.noise_rate)
    ...
    opts = [SingleDeviceMuon(muon_params, lr=0.02, momentum=0.95, weight_decay=0.01),
            torch.optim.AdamW(adam_params, lr=3e-3, weight_decay=0.0)]
    ...
    for step in range(args.steps):
        ...
        # Teacher logits (no grad, frozen)
        with torch.no_grad():
            teacher_logits = teacher.teacher_logits(det, dc, syn_mask)
        ...
        with torch.autocast("cuda", dtype=torch.bfloat16):
            ...
            # KL divergence via soft target match
            t_prob = torch.sigmoid(teacher_logits / T)
            loss_kl = F.binary_cross_entropy_with_logits(student_logits / T, t_prob) * (T * T)
        ...
        if step > 0 and step % args.eval_interval == 0:
            ...
            if ler < best:
                ...
                torch.save({'step': step, 'model_state_dict': student.state_dict(),
                            'config': config, 'ler': ler, 'distilled_from_lange': True},
    ...
    torch.save({'step': args.steps, 'model_state_dict': student.state_dict(),
                'config': config, 'distilled_from_lange': True},
