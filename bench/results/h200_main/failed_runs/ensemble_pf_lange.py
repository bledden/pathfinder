...
class LangeWrapper:
    def __init__(self, d, d_t, m_nearest_nodes=10, power=2):
        ...
        self.model = GNN_7(
            ...
            hidden_channels_MLP=([512,256,128,64,32,16] if self.d==9 else [256,128,64]), num_classes=1,
        ).to(device)
    ...
    def predict_batch(self, det):
        ...
        with torch.no_grad():
            out = self.model(x, ei, batch, edge_attr)
...
def load_pathfinder(d, ckpt_dir):
    ...
    if d == 7:
        candidates.append(f"{ckpt_dir}/d{d}_final/best_model.pt")
    elif d in (3, 5):
        candidates.append(f"{ckpt_dir}/d{d}_muon/best_model.pt")
    for p in candidates:
        if os.path.exists(p):
...
def main():
    ...
    for d in args.distances:
        for p in args.noise_rates:
            ...
            # Incremental save after each (d, p) point — survive pod death
            with open(args.output, "w") as f:
                json.dump(results, f, indent=2)
            print(f'  [incremental save: {len(results)} points to {args.output}]', flush=True)
